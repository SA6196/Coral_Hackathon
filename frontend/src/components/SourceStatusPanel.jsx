...
function StatusDot({ status }) {
  const c = status === "connected" ? "#00ff9d" : status === "not_configured" ? "#fbbf24" : "#ef4444";
  ...
  return (
    <section style={{ marginBottom: 20 }} aria-label="Source Status Panel">
      ...
      >
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: "#00d4ff" }}>Coral Data Sources</span>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {sources.map(s => {
            const c = s.status === "connected" ? s.color : s.status === "not_configured" ? "#fbbf24" : "#ef4444";
        ...
        <button
          ...
          style={{
            ...
            color: "#00d4ff", fontSize: 10, display: "flex", alignItems: "center", gap: 3,
          }}
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 10, marginBottom: 12 }}>
                    {sources.map((s, i) => {
                      const c = s.status === "connected" ? "#00ff9d" : s.status === "not_configured" ? "#fbbf24" : "#ef4444";
                      ...
                      return (
                        <motion.div key={s.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            ...
                            {s.is_public && <span style={{ fontSize: 8, padding: "1px 5px", borderRadius: 3, background: "rgba(0,255,157,0.1)", border: "1px solid rgba(0,255,157,0.2)", color: "#00ff9d" }}>PUBLIC</span>}
                          </div>
                          ...
                          <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.35)", lineHeight: 1.8 }}>
                            <div>Table: <code style={{ color: "#00d4ff" }}>{s.coral_table}</code></div>
                            <div>Join: <code style={{ color: "#a855f7", fontSize: 9 }}>{s.join_key}</code></div>
                            <div>Synced: {rel(s.last_synced)}</div>
                            {s.env_var && <div>Env: <code style={{ color: "#fbbf24" }}>{s.env_var}</code></div>}
                          </div>
                        </motion.div>
                      );
                    })}
                  ...
                  <button onClick={() => setSqlVis(v => !v)} style={{
                    ...
                    fontSize: 10, color: "rgba(0,212,255,0.5)", display: "flex", alignItems: "center", gap: 4, padding: "2px 0",
                  }}>
                  ...
                  <AnimatePresence>
                    {sqlVis && sql && (
                      <motion.pre initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                        style={{ fontSize: 9.5, background: "rgba(0,0,0,0.4)", border: "1px solid rgba(0,212,255,0.15)", borderRadius: 8, padding: "10px 14px", marginTop: 8, color: "#00d4ff", overflow: "hidden" }}>
                        {sql}
                      </motion.pre>
                    )}
                  </AnimatePresence>
